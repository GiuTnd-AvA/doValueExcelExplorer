...
TMP = os.path.join(WORK, 'tmp_test_table_ddl_fallback')
...
# Fake pyodbc connect with cursor that fails on first execute (STRING_AGG) then succeeds
class FakeCursor:
    def __init__(self):
        self.calls = 0
    def execute(self, tsql, params):
        self.calls += 1
        if 'STRING_AGG' in tsql and self.calls == 1:
            raise Exception('STRING_AGG unsupported')
        # second call uses XML PATH and should pass
    def fetchone(self):
